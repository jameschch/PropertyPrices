...
propertyData <- propertyData[, c("Date", "FlatPrice", "RegionName", "AreaCode")][order(as.Date(propertyData$Date, format = "%d/%m/%Y")),]

#londonOnly <- subset(propertyData, RegionName %in% london)
londonOnly <- subset(propertyData, startsWith(as.character(AreaCode), "E09"))

p1 = plot_ly(londonOnly, x = as.Date(londonOnly$Date, format = "%d/%m/%Y"), y = londonOnly$FlatPrice, split = londonOnly$RegionName, type = "scatter", mode = "lines")

p1 = layout(p1, plot_bgcolor = "#000", paper_bgcolor = "#000", font = list(color = "white"))
